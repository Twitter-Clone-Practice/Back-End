...
describe 'Fetch user liked Posts' do
    before :each do
        ...
        @second_post = Post.create!(user_id: @user_eternal.id, message: "Second Post", number_of_comments: 2, number_of_likes: 0)
        @crimson_post = Post.create!(user_id: @user_crimson.id, message: "Cool", number_of_comments: 0, number_of_likes: 0)
    ...
    it "should be able to add to post number of likes when a user likes the post" do
        query_string = <<-GRAPHQL
            mutation{
                likePost(input: {
                    postId: #{ @crimson_post.id }
        ...
        expect(Like.all.count).to eq(1)
        expect(@crimson_post.number_of_likes).to eq(0)
        ...
        expect(Like.all.count).to eq(2)
        expect(@crimson_post.number_of_likes).to eq(1)
    end
